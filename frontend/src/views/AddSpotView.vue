  ...
  <main>
    ...
    <div class="images">
      <div
        ...
        class="image"
      >
        <span class="removeImg" @click="removeImage" :id="index.toString()">
...
function getLocation() {
  if (window.navigator.geolocation) {
    ...
    window.navigator.geolocation.getCurrentPosition((position: GeolocationPosition) => {
      ...
      const lon =
        Math.round(position.coords.longitude * POSITIONING_ACCURACY) / POSITIONING_ACCURACY
...
function removeImage(event: Event) {
  ...
  spot.value.images.splice(index, 1)
}
...
function submit() {
  ...
  if (
    spot.value.title.length > 0 &&
    spot.value.latitude != 0 &&
    spot.value.longitude != 0 &&
    spot.value.images.length > 0
  ) {
    tripStore.addSpotToTrip(tripId, spot.value).then(() => (window.location.href = '/'))
  } else {
